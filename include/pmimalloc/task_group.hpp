...
namespace threading {
    ...
    struct priority_task
    {
        ...
        priority_task(task&& t, int priority)
          : t(std::move(t))
          , priority(priority)
        {
        }
    ...
    public:
        ...
        void async(task t, int priority)
        {
            async({std::move(t), priority});
        }
        void run(task t, int priority)
        {
            run({std::move(t), priority});
        }
    ...
    public:
        ...
        template <typename F>
        wrap<callable<F>>
        make_wrapped_function(F&& f, std::atomic<std::size_t>& c, exception_state& ex)
        {
        ...
        void run(F&& f, int priority)
        {
            running_ = true;
            ++in_flight_;
            task_system_->async(priority_task{
                make_wrapped_function(std::forward<F>(f), in_flight_, exception_status_),
                priority});
        }
