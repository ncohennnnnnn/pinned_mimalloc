...
struct resource_builder
{
    ...
    constexpr auto is_simple(void) const
    {
        ...
        return updated<0, simple>(std::tuple<>{});
    }

    constexpr auto is_mirrored(void) const
    {
        // mirrored resources stored at position 0 in the resource nest
        return updated<0, mirrored>(std::tuple<>{});
    }
    ...
    constexpr auto use_mimalloc(void) const
    {
        ...
        return updated<1, resource, ext_mimalloc>(std::tuple<>{});
    }
#endif
    ...
    {
        // arena resources are stored at position 1 in the resource nest
        return updated<1, resource, ext_stdmalloc>(std::tuple<>{});
    }
    ...
    constexpr auto register_memory(void) const
    {
        ...
        return updated<2, context, backend>(std::tuple<>{});
    }
#endif
    ...
    {
        // pinned resources are stored at position 3 in the resource nest
        return updated<3, cuda_pinned>(std::tuple<>{});
    }
#endif
    ...
    {
        // pinned resources are stored at position 3 in the resource nest
        return updated<3, not_pinned>(std::tuple<>{});
    }
    ...
    constexpr auto on_host(const std::size_t size, const int alignment = 0) const
    {
        // memory resources are stored at position 4 in the resource nest
        return updated<4, host_memory>(std::make_tuple(size, alignment));
    }

    constexpr auto on_mirror(const std::size_t size, const int alignment = 0) const
    {
        // memory resources are stored at position 4 in the resource nest
        auto tmp = is_mirrored();
        // updated<4, mirror_memory>(std::make_tuple(size, alignment));
        return tmp.template updated<4, mirror_memory>(std::make_tuple(size, alignment));
        // return updated<4, mirror_memory>(std::make_tuple(size, alignment));
    }

    constexpr auto use_host_memory(void* ptr, const std::size_t size) const
    {
        // memory resources are stored at position 4 in the resource nest
        return updated<4, user_host_memory>(std::make_tuple(ptr, size));
    }
    ...
    // constexpr auto use_mirror_memory(
    //     void* ptr, const std::size_t size, const int alignment = 0) const
    // {
    //     // memory resources are stored at position 4 in the resource nest
    //     return updated<4, user_mirror_memory>(std::make_tuple(ptr, size, alignment));
    // }
