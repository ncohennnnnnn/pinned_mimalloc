...
#ifdef NUMA_THROWS
# include <stdexcept>
# define NUMA_ERROR(MSG) throw std::runtime_error("libnuma error: " MSG)
#else
# include <iostream>
# define NUMA_ERROR(MSG) std::cerr << "libnuma error: " << MSG << "\n"
#endif
...
numa_tools::numa_tools() NUMA_CONDITIONAL_NOEXCEPT
{
    // initialize libnuma
    if (numa_available() < 0)
        NUMA_ERROR("could not initialize libnuma");
    else
    {
...
void numa_tools::discover_nodes() noexcept
{
    ...
    for (index_type i = 0; i < num_nodes; ++i)
    {
        ...
        if (numa_bitmask_isbitset(numa_all_nodes_ptr, i))
        {
            ...
            if (numa_bitmask_weight(cpu_mask_ptr) > 0)
                host_nodes_.push_back(i);
            else
...
numa_tools::index_type numa_tools::local_node() const noexcept
{
    return m_cpu_to_node[sched_getcpu()];
}

bool numa_tools::can_allocate_on(index_type node) const noexcept
{
    return local_nodes().count(node) > 0u;
}

numa_tools::allocation numa_tools::allocate(size_type num_pages) const noexcept
{
    return allocate(num_pages, local_node());
}

numa_tools::allocation numa_tools::allocate(size_type num_pages, index_type node) const noexcept
{
    if (num_pages == 0u)
        return {};
#ifndef NUMA_FOR_LOCAL
    ...
    if (!ptr)
        return allocate_malloc(num_pages);
    LOG("allocating", num_pages * page_size_, "bytes using numa_alloc:", (std::uintptr_t) ptr);
    return {ptr, num_pages * page_size_, node};
}

numa_tools::allocation numa_tools::allocate_malloc(size_type num_pages) const noexcept
{
    void* ptr = std::calloc(num_pages, page_size_);
    //void* ptr = std::malloc(num_pages * page_size_);
    if (!ptr)
        return {};
    LOG("allocating", num_pages * page_size_, "bytes using std::malloc:", (std::uintptr_t) ptr);
    return {ptr, num_pages * page_size_, get_node(ptr), false};
}

numa_tools::index_type numa_tools::get_node(void* ptr) const noexcept
{
    int node_id = 0;
...
void numa_tools::free(numa_tools::allocation const& a) const noexcept
{
    if (a)
    {
        if (a.use_numa_free)
        {
            LOG("freeing   ", a.size, "bytes using numa_free:", (std::uintptr_t) a.ptr);
            numa_free(a.ptr, a.size);
        }
        else
        {
            LOG("freeing   ", a.size, "bytes using std::free:", (std::uintptr_t) a.ptr);
            std::free(a.ptr);
        }
    }
}
...
numa_tools make_numa_tools() NUMA_CONDITIONAL_NOEXCEPT
{
    return {};
}
